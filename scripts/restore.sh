...
Example: ./popcorn.sh ./helloworld ubuntu@172.31.23.242 ./hcontainer_check [-p] 1111:1111
Author: Tong Xing
Stevens Institute of Technology 2020
EOF
    exit 0
...
if ssh $TARGET_MACHINE test -e $RDIR;
	then ssh $TARGET_MACHINE rm -r $RDIR
fi
...
RCID=$(ssh $TARGET_MACHINE "bash -s" < ./builder.sh $RDIR $PORT_FLAG $PORT_BIND)
